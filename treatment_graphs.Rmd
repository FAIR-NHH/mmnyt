...
ipsos <- haven::read_dta(here::here("data","processed", "final_ipsos_cleaned.dta")) %>%
 mutate( year=2020,
         country="United States",
         state = as.character(haven::as_factor(State_Recoded)),
         inequality_luck = ifelse(luck_d, inequality, NA),
...
saved to disk.
```{r}
tgraphs <- gsol + gnat + glunfair +  plot_layout(ncol=1, guides = 'collect') +
  plot_annotation(tag_levels = 'a')
tgraphs
ggsave(here::here("graphs","treatment_effects.pdf"), width=16, height=20, units="cm")
```
...
outcome_means <- ipsos %>% 
  group_by(state) %>%
  mutate( inequality_luck = ifelse(luck_d, inequality, NA)) %>%
  summarize(
    mean_happiness = weighted.mean(happiness_ladder, weight, na.rm=TRUE),
