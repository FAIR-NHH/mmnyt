...
nationalism_hist <- nationalism_hist_df %>%
  ...
  labs(x=element_blank(),
       y="Proportion",
       title="Nationalism")
noborders_hist_df <- ipsos %>% 
  summarize(weighted.bars(6-noborders_agree, Weightvar, na.rm=TRUE))
noborders_hist_df %>% knitr::kable(digits=3)
noborders_hist <- noborders_hist_df %>%
  ggplot(aes(x=v, y=proportion)) + 
  geom_bar(stat='identity') +
  theme_minimal() +
  scale_x_continuous(breaks=1:5) +
...
outcomes_hist <- solidarity_hist + nationalism_hist + luckunfair_hist +
  compassion_hist + noborders_hist + luckdetermine_hist + 
...
severe2 <- ipsos_with_avg %>% lm( zsolidarity ~ corona_prime + prime_many_affected + many_affected +
                                    ...
                                    child + alone + urban + high_confirmed +
                                    northeast + midwest + south, data=., weights=Weightvar)        
severe3 <- ipsos_with_avg %>% lm( zluck_unfair ~ corona_prime + prime_expecting_long + expecting_long +
                                    retirement_age + female + highinc + republican + higheduc +
                                    child + alone + urban + high_confirmed +
                                    northeast + midwest + south, data=., weights=Weightvar)        
severe4 <- ipsos_with_avg %>% lm( zluck_unfair ~ corona_prime + prime_many_affected + many_affected +
                                    retirement_age + female + highinc + republican + higheduc +
                                    child + alone + urban + high_confirmed +
                                    northeast + midwest + south, data=., weights=Weightvar)
```
...
```{r}
stargazer(severe1, severe2, severe3, severe4, 
          se = list(
            sqrt(diag(vcovHC(severe1, type = "HC1"))),
            sqrt(diag(vcovHC(severe2, type = "HC1"))),
            sqrt(diag(vcovHC(severe3, type = "HC1"))),
            sqrt(diag(vcovHC(severe4, type = "HC1")))),
          style="aer",type="text", df=FALSE, keep.stat=c("rsq","n"))
```
